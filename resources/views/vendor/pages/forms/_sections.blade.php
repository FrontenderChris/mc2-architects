{!! pageOpenForm($form, (isset($model) ? $model : null)) !!}
    <ul class="nav nav-tabs">
        ...
        <li class="{{ request()->has('section') ? 'active' : '' }}"><a href="#" class="{{ (empty($page) ? 'disabled-tab' : 'do-show-content') }}" data-show=".tab-sections">Sections</a></li>
    </ul>
    ...
    @if (!empty($page))
        @include('sections::partials._index', ['page' => $page])
    @endif
    {{------------------------------ EDIT THE FORM ABOVE ------------------------------}}
